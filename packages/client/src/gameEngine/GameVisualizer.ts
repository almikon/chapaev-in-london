...
export class GameVizualiser {
  ...
  public start = () => {
    ...
    const resizeObserver = new ResizeObserver((entries) => {
      for (const entry of entries) {
        const width = entry.contentBoxSize?.length
          ? entry.contentBoxSize[0].inlineSize
          : entry.contentRect.width;
        this.adjustCanvasDimensions(width);
      }
      this.drawBoard();
    });
  ...
  private onMouseMove = (e: MouseEvent) => {
    ...
    this._gameInteraction.mousePosition = this.canvasCoordToVector(this._mousePosition);
  };
  ...
  drawBoard = () => {
    ...
    for (let y = marginY; y < this.boardCanvas.height - marginY; y += stepY) {
      colorSwitch = !colorSwitch;
      for (let x = marginX; x < this.boardCanvas.width - marginX; x += stepX) {
  ...
  private animateDestroyed = async (tickResult: GameStepResult) => {
    tickResult.destroyedCheckers.forEach(async (checker) => {
      ...
      for (let i = 0; i <= 80; i++) {
        const dx = (Math.random() - 0.5) * ((Math.random() * this.canvas.width) / 50);
        const dy = (Math.random() - 0.5) * ((Math.random() * this.canvas.height) / 50);
  ...
  private drawChecker = (checker: Checker) => {
    ...
    if (checker.selected) {
      this.context.beginPath();
      this.context.arc(canvasPos.x, canvasPos.y, canvasRadius + 2, 0, 2 * Math.PI);
